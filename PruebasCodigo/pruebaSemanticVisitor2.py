...
class SemanticVisitor(yalpVisitor):
    ...
    def visitExpr(self, ctx: yalpParser.ExprContext):
        ...
        if ctx.IF():
            ...
            inherit_visited = []
            native_classes = ['Int', 'Bool', 'String', 'IO']
            visited_native = []
            ...
            def GetFunc(scope, func, inherits):
                if func in scope.symbols:
                    ...
                    elif inherits in inherit_visited:
                        if inherits !="Object":
                            type_symbol = self.tablaSimbolos.get_scope_simbolo(inherits)
                            message = f'Semantic Error semántico: Hay una herencia recursiva en la clase {inherits}, {ctx.start.line}:{ctx.start.column}'
                            self.errors.append(message)
                            return(False, None, None)
                else:
                    while native_classes:
                        native = native_classes.pop()

                        if native not in inherit_visited:
                            inherit_visited.append(native)
                            type_symbol = self.tablaSimbolos.get_scope_simbolo(native)
                            scope_tipo = type_symbol.scope
                            scope_tipo = scope_tipo.get_symbol_scope(type_symbol)
                            if scope_tipo[1] not in inherit_visited:
                                return GetFunc(scope_tipo[0], func, scope_tipo[1])
                            else:
                                return GetFunc(scope_tipo[0], func, None)
                        else:
                            type_symbol = self.tablaSimbolos.get_scope_simbolo(native)
                            message = f'Semantic Error semántico: Hay una herencia recursiva en la clase {native}, {ctx.start.line}:{ctx.start.column}'
                            self.errors.append(message)
                            return(False, None, None)
                    else:
                        return (False, None, None)    
                ...
                if token_type=='ID':
                    ...
                    if not simbolo:
                        ...
                        if not found:
                            ...
                            if input_types != None:
                                print(type_symbol.lexema, found, tipo_func, arg_tipos)
                                input()
