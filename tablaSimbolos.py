...
class Simbolo:
    def __init__(self, lexema, linea, columna, tipo_token, scope, parametro=False, hereda=None, funcion=False, in_function = False):
    ...
    def __str__(self):
        return f"Lexema: {self.lexema}, Linea: {self.linea}, Columna: {self.columna}, Tipo de Token: {self.tipo_token}, Scope: {self.scope}, Parametro: {self.parametro}, Hereda: {self.hereda}, Funcion: {self.funcion}, Tamaño (bytes): {self.size}, Offset: {self.offset}, In_Function: {self.in_function}"
...
class Scope:
    ...
    def print_scope(self, level=0):
        ...
        x = PrettyTable()
        x.field_names = ["Lexema", "Linea", "Columna", "Tipo de Token", "Global", "Parametro", "Hereda", "Funcion", "Tamaño (bytes)", "Offset", "In Function"]
        for name, simbolo in self.symbols.items():
            x.add_row([simbolo.lexema, simbolo.linea, simbolo.columna, simbolo.tipo_token, simbolo.global_, simbolo.parametro, simbolo.hereda, simbolo.funcion, simbolo.size, simbolo.offset, simbolo.in_function])
